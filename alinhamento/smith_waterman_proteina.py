...
def alinhamento(seq1, seq2, gap_penalty=-8):
    ...
    for i in range(1, len(seq1) + 1):        
        for j in range(1, len(seq2) + 1):            
            mutacao = seq1[i-1]+seq2[j-1]            
            match = score_matrix[i-1][j-1] + blosum62.get(mutacao, -3)            
    ...
    while score_matrix[i][j] != 0:        
        mutacao = seq1[i-1]+seq2[j-1]        
        if score_matrix[i][j] == score_matrix[i-1][j-1] + blosum62.get(mutacao, -3):            
...
# Exemplo de uso
seq1 = "YAEDKEDEWKAPLA"
seq2 = "MACHIPYYWEDKREWKPLASWNVCAAACLIKDKED"
score, aligned_seq1, aligned_seq2 = alinhamento(seq1, seq2)
